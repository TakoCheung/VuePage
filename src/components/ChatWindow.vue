<template>
	<div class="window-container" v-visible="isPanelOpen" :aria-hidden="!isPanelOpen" :class="{ 'window-mobile': isDevice }">
    ...
    closeChatPanel: toggle.toggleChat,
		...
		async fetchMoreRooms() {
			...
			Object.keys(roomList).forEach(key => {
				...
				room.roomName =
					roomContacts.map(user => user.username).join(', ') || 'Welcome'

				const roomAvatar =
					roomContacts.length === 1 && roomContacts[0].avatar
						? roomContacts[0].avatar
						: require('@/assets/self.jpg')
